...
public class FKaristir implements Serializable {
    ...
    public FKaristir(MyImage[][] resimler, int level){
        ...
        for(int i=0; i<resimler.length; i++){
            for(int j=0; j<resimler.length; j++){
                if(resimler[i][j].get_id() == 0){//bos resimdeyse
                    sayaclar[i][j]=1;
                }else{
                    sayaclar[i][j]=0;
                }
    ...
    private void karistir(int level){
        ...
        for(int i=1; i<=iterasyon_sayisi; i++){//karistirmada kullanilacak adim sayisi == i
            ...
            if(Y_Yonler.ASAGI.get_is_tanimli()){
                ...
                MyImage temp = resimler[bos_resim_index_satir+1][bos_resim_index_sutun];//asagidaki sayiyi al
                resimler[bos_resim_index_satir][bos_resim_index_sutun]=temp;//bos resmin yerine yaz
                resimler[bos_resim_index_satir+1][bos_resim_index_sutun].set_id(0);//asagidaki resmi bos yap
                
                System.out.println("ASAGI YONUNDE KARISTIRMA YAPILDI");
            }else if(Y_Yonler.YUKARI.get_is_tanimli()){
                //Yukari ile karistir ve kaydet
                MyImage temp = resimler[bos_resim_index_satir-1][bos_resim_index_sutun];//yukaridaki sayiyi al
                resimler[bos_resim_index_satir][bos_resim_index_sutun]=temp;//bos resmin yerine yaz
                resimler[bos_resim_index_satir-1][bos_resim_index_sutun].set_id(0);//yukaridaki resmi bos yap
                
                System.out.println("YUKARI YONUNDE KARISTIRMA YAPILDI");
            }else if(X_Yonler.SAG.get_is_tanimli()){
                //Sag ile karistir ve kaydet
                MyImage temp = resimler[bos_resim_index_satir][bos_resim_index_sutun+1];//sagdaki sayiyi al
                resimler[bos_resim_index_satir][bos_resim_index_sutun]=temp;//bos resmin yerine yaz
                resimler[bos_resim_index_satir][bos_resim_index_sutun+1].set_id(0);//sagdaki resmi bos yap
                
                System.out.println("SAG YONUNDE KARISTIRMA YAPILDI");
            }else if(X_Yonler.SOL.get_is_tanimli()){
                //Sol ile karistir ve kaydet
                MyImage temp = resimler[bos_resim_index_satir][bos_resim_index_sutun-1];//soldaki sayiyi al
                resimler[bos_resim_index_satir][bos_resim_index_sutun]=temp;//bos resmin yerine yaz
                resimler[bos_resim_index_satir][bos_resim_index_sutun-1].set_id(0);//soldaki resmi bos yap
